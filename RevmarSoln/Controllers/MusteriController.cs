...
namespace SahinRektefiyeSoln.Controllers
{
    ...
    public class MusteriController : BaseController
    {
            ...
            if (MusteriId != 0)
            {
                ...
                if (editEdilecekMusteri.MusteriMail != null)
                {
                    if (editEdilecekMusteri.MusteriMail.Count > 0)
                        model.Mail = editEdilecekMusteri.MusteriMail.Select(x => x.MailAdresi).ToList();
                }
                ...
                if (editEdilecekMusteri.MusteriTelefon != null)
                {
                    if (editEdilecekMusteri.MusteriTelefon.Count > 0)
                        model.Telefon = editEdilecekMusteri.MusteriTelefon.Select(x => x.TelefonNumarasi).ToList();
                }
        ...
        public ActionResult MusteriDuzenle(MusteriEditPageModel model)
        {
            ...
            //db'den gelen telefon numaraları ve mailler id'ye göre sıralandı ve farklı olanlar güncellendi.
            List<string> tempPhoneList = musteri.MusteriTelefon.OrderBy(x => x.MusteriTelefonId).Select(x => x.TelefonNumarasi).ToList();
            for (var i = 0; i < model.Telefon.Count; i++)
            {
                if (tempPhoneList[i] != model.Telefon[i])
                {
                    musteri.MusteriTelefon.FirstOrDefault(x => x.TelefonNumarasi == tempPhoneList[i]).TelefonNumarasi = model.Telefon[i];
                }
            }

            List<string> tempMailList = musteri.MusteriMail.OrderBy(x => x.MusteriMailId).Select(x => x.MailAdresi).ToList();
            for (var i = 0; i < model.Mail.Count; i++)
            {
                if (tempMailList[i] != model.Mail[i])
                {
                    musteri.MusteriMail.FirstOrDefault(x => x.MailAdresi == tempMailList[i]).MailAdresi = model.Mail[i];
                }
            }

            musteri.Email = model.Mail[0]; //İlk eleman atıldı.

            //Yeni Tel veya Mail eklendiyse ilgili tablolara kayıtlar atılır.
            if (!string.IsNullOrEmpty(model.NewPhoneNumber))
            {
                MusteriTelefon yeniMusteriTelefon = new MusteriTelefon();
                yeniMusteriTelefon.TelefonNumarasi = model.NewPhoneNumber;
                yeniMusteriTelefon.SwOnaySms = false;
                yeniMusteriTelefon.SwOnayArama = false;
                yeniMusteriTelefon.SwPrefered = true;
                yeniMusteriTelefon.MusteriId = model.musteri.MusteriId;
                ...
                db.MusteriTelefon.Add(yeniMusteriTelefon);
                db.SaveChanges();
            }

            if (!string.IsNullOrEmpty(model.NewMail))
            {
                MusteriMail yeniMusteriMail = new MusteriMail();
                yeniMusteriMail.SwOnay = false;
                yeniMusteriMail.MailAdresi = model.NewMail;
                yeniMusteriMail.MusteriId = model.musteri.MusteriId;

                yeniMusteriMail.DtCreated = System.DateTime.Now;
                yeniMusteriMail.Creator = currentUser;

                db.MusteriMail.Add(yeniMusteriMail);
                db.SaveChanges();
            }
        ...
        [HttpPost]
        public ActionResult RemovePhoneNumber(string phoneNumber, int id)
        {
            try
            {
                MusteriTelefon musteriTelefon = db.MusteriTelefon.FirstOrDefault(x => x.MusteriId == id && x.TelefonNumarasi == phoneNumber);
                db.MusteriTelefon.Remove(musteriTelefon);
                db.SaveChanges();
                return Json(true);
            }
            catch (Exception)
        ...
        [HttpPost]
        public ActionResult RemoveMail(string mail, int id)
        {
            try
            {
                MusteriMail musteriMail = db.MusteriMail.FirstOrDefault(x => x.MusteriId == id && x.MailAdresi == mail);
                db.MusteriMail.Remove(musteriMail);
                db.SaveChanges();
                return Json(true);
            }
            catch (Exception)
        ...
        {
            List<Musteri> musteri = db.Musteri.Where(x => x.MusteriAdi.Contains(search)).ToList();
            if (musteri != null)
            {
                return PartialView("_SearchCustomerByName", musteri);
            }
            return Json(false);
        }
